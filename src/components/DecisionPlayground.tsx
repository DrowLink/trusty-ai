    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-8 space-y-10 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 space-y-6">
          <div className="rounded-xl bg-[#090d16] border border-slate-800 p-6 space-y-5">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="text-xs font-mono font-bold uppercase tracking-widest text-slate-300 flex items-center space-x-2">
                <Cpu className="w-4 h-4 text-sky-400" />
                <span>SIMULATE ECONOMIC ACTION</span>
              </div>
              <span className="text-[10px] font-mono text-slate-400">POST /api/decision</span>
            </div>
            ...
            <form onSubmit={handleEvaluate} className="space-y-4 text-xs font-mono">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded px-3 py-2 text-white font-mono focus:border-sky-500 focus:outline-none"
                >
                  {agents.map(a => (
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-slate-400 uppercase text-[10px]">
                    AMOUNT (USD)
                  </label>
                  <div className="space-x-1">
                    {[100, 840, 5200, 30000].map(val => (
                      <button
                        ...
                        className="px-1.5 py-0.5 rounded bg-slate-800/80 hover:bg-slate-700 text-[9px] text-slate-300"
                      >
                        ${val.toLocaleString()}
                      </button>
                    ))}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded pl-7 pr-3 py-2 text-white font-mono focus:border-sky-500 focus:outline-none"
                  />
                </div>
              </div>
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="w-full mt-4 py-3 rounded bg-purple-600 hover:bg-purple-500 disabled:opacity-50 text-white font-bold text-xs font-mono transition flex items-center justify-center space-x-2 shadow-lg shadow-purple-900/20"
              >
                {isLoading ? (
                  <>
          ...
          {/* Quick Info box */}
          <div className="p-4 rounded-xl bg-slate-900/50 border border-slate-800 text-[11px] font-mono text-slate-400 space-y-1 leading-relaxed">
            <div className="text-slate-300 font-bold uppercase text-[10px]">Machine-Speed SLA</div>
            <p>
              Average risk evaluation latency: <strong className="text-emerald-400">18ms</strong>. Runs synchronous policy checks against deterministic 20-signal AST telemetry and credit capacity envelopes.
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="rounded-xl bg-[#090d16] border border-slate-800 p-6 space-y-6 relative overflow-hidden">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="text-xs font-mono font-bold uppercase tracking-widest text-slate-300 flex items-center space-x-2">
            ...
            {decisionResult ? (
              <div className="space-y-6">
                ...
                <div className="p-4 rounded-lg bg-[#060c14] border border-sky-900/40 space-y-1 font-mono text-xs">
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {decisionResult.policyChecks.map((check, idx) => (
                      <div
                        key={idx}
                        className="p-2.5 rounded bg-slate-950/80 border border-slate-800/80 flex items-start justify-between gap-3 text-xs font-mono"
                      >
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            <span className="text-slate-200 font-bold">{check.rule}: </span>
                        ...
                        <span className={`text-[10px] font-bold uppercase px-1.5 py-0.5 rounded flex-shrink-0 ${
                          check.passed ? 'bg-emerald-950 text-emerald-400' : check.severity === 'warning' ? 'bg-amber-950 text-amber-400' : 'bg-rose-950 text-rose-400'
                        }`}>
              ...
              <div className="py-12 text-center text-slate-500 font-mono text-xs space-y-3">
                <CreditCard className="w-10 h-10 mx-auto text-slate-600 opacity-60" />
          ...
          <div className="rounded-xl bg-[#090d16] border border-slate-800 overflow-hidden">
            <div className="flex items-center justify-between px-4 py-2.5 bg-slate-950 border-b border-slate-800">
              <div className="flex items-center space-x-2 text-xs font-mono">
                ...
                <span className="text-slate-400 font-bold uppercase">SDK Snippet:</span>
              ...
              <button
                ...
                className="flex items-center space-x-1 text-[11px] font-mono text-slate-400 hover:text-white transition"
              >
            ...
            <pre className="p-4 text-xs font-mono text-slate-300 overflow-x-auto bg-[#06080e] leading-relaxed">
