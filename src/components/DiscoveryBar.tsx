...
export const DiscoveryBar: React.FC<DiscoveryBarProps> = ({ onRefresh }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto mt-4 mb-8 px-3 sm:px-6">
      <div className="security-card p-3 sm:p-3.5 rounded-xl flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5">
        {/* Left: Sources */}
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex items-center space-x-1.5 text-xs font-mono text-slate-400 mr-1">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span className="font-semibold text-slate-200">DISCOVERY ENGINE:</span>
          </div>

          <div className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-lg bg-slate-900 border border-slate-800 text-[10px] sm:text-[11px] font-mono text-slate-300">
        ...
        <div className="flex items-center w-full sm:w-auto justify-end">
          <button
            ...
            disabled={isCrawling}
            className="w-full sm:w-auto flex items-center justify-center space-x-2 px-3.5 py-1.5 rounded-lg text-xs font-mono font-medium text-slate-200 bg-slate-800 hover:bg-slate-700 active:bg-slate-900 border border-slate-700/80 disabled:opacity-50 transition shadow-sm"
          >
      ...
      {lastCrawlResult && (
        <div className="mt-2.5 text-[11px] font-mono text-emerald-300 flex items-center space-x-1.5 bg-emerald-950/40 px-3 py-2 rounded-lg border border-emerald-800/50">
