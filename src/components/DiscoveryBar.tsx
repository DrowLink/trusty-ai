...
export const DiscoveryBar: React.FC<DiscoveryBarProps> = ({ onRefresh }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto my-6">
      <div className="glass-panel p-3.5 sm:p-4 rounded-xl border border-slate-800/90 flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        {/* Left: Ecosystem Sources indicator */}
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          <div className="flex items-center space-x-1.5 text-xs font-mono text-slate-300 mr-1">
            <Radio className="w-3.5 h-3.5 text-emerald-400 animate-pulse" />
            <span className="font-semibold">Discovery Engine:</span>
          </div>

          <div className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md bg-slate-900 border border-slate-700/60 text-[11px] text-slate-300">
            <Github className="w-3 h-3 text-sky-400" />
          ...
          <div className="inline-flex items-center space-x-1.5 px-2.5 py-1 rounded-md bg-slate-900 border border-slate-700/60 text-[11px] text-slate-300">
            ...
            <span>HuggingFace & CrewAI Hubs</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3 w-full md:w-auto justify-end">
          <button
            ...
            disabled={isCrawling}
            className="flex items-center space-x-2 px-4 py-2 rounded-lg text-xs font-medium text-slate-200 bg-slate-800 hover:bg-slate-700 border border-slate-600 disabled:opacity-50 transition"
          >
            <RefreshCw className={`w-3.5 h-3.5 text-sky-400 ${isCrawling ? 'animate-spin' : ''}`} />
            <span>{isCrawling ? 'Crawling Ecosystems...' : 'Trigger Multi-Source Crawl'}</span>
          </button>
        </div>
      </div>

      {lastCrawlResult && (
        <div className="mt-2 text-xs font-mono text-emerald-400/90 flex items-center space-x-1.5 bg-emerald-950/30 px-3 py-1.5 rounded-lg border border-emerald-800/40">
